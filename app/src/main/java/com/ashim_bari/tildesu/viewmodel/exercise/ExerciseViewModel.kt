...
class ExerciseViewModel @Inject constructor(
    ...
    init {
        ...
        Log.d(
            TAG,
            "Initial values: quizScore=${_quizScore.value}, trueFalseScore=${_trueFalseScore.value}, puzzleScore=${_puzzleScore.value}, exerciseCompleted=${_exerciseCompleted.value}, quizPassed=${_quizPassed.value}, ImageQuizPassed=${_imageQuizPassed.value}"
        )
    }
        ...
        viewModelScope.launch {
            try {
                ...
                if (exercisesList.isNotEmpty()) {
                    ...
                    Log.d(
                        TAG,
                        "Exercises loaded for level: $level, type: $type, total: ${exercisesList.size}"
                    )
                } else {
    ...
    fun submitTrueFalseAnswer(userAnswer: Boolean) {
        ...
        Log.d(
            TAG,
            "Attempting True/False answer. Current type: ${currentExercise?.type}, Index: ${_currentExerciseIndex.value}"
        )
        if (currentExercise?.type != ExerciseType.TRUE_FALSE) {
            Log.d(
                TAG,
                "Non-True/False exercise attempted in True/False method. Exercise type: ${currentExercise?.type}"
            )
            return
        }
                    ...
                    try {
                        ...
                        val updatedData = when (currentExerciseType) {
                            ...
                            ExerciseType.IMAGE_QUIZZES -> mapOf(
                                "imageQuizCorrect" to min(
                                    _imageQuizScore.value ?: 0,
                                    _exercises.value?.size ?: 0
                                ),
                        ...
                        if (updatedData.isNotEmpty()) {
                            val commonData = mapOf(
                                "overallTotal" to calculateOverallTotalQuestions(
                                    currentProgress,
                                    updatedData
                                ),
                                "overallCorrect" to calculateOverallCorrectAnswers(
                                    currentProgress,
                                    updatedData
                                ),
                            ...
                            Log.d(
                                TAG,
                                "Progress updated for user: $userId, level: $levelId with data: $updatedData"
                            )
                        }
                    } catch (e: Exception) {
                        Log.e(
                            TAG,
                            "Failed to update progress for user: $userId, level: $levelId",
                            e
                        )
                    }
                }
    ...
    private fun calculateOverallTotalQuestions(
        currentProgress: Map<String, Any>,
        updatedData: Map<String, Any>
    ): Int {
        val quizTotal =
            (updatedData["quizTotal"] as? Number ?: currentProgress["quizTotal"] as? Number
            ?: 0).toInt()
        val trueFalseTotal = (updatedData["trueFalseTotal"] as? Number
            ?: currentProgress["trueFalseTotal"] as? Number ?: 0).toInt()
        val puzzleTotal =
            (updatedData["puzzleTotal"] as? Number ?: currentProgress["puzzleTotal"] as? Number
            ?: 0).toInt()
        val imageQuizTotal = (updatedData["imageQuizTotal"] as? Number
            ?: currentProgress["imageQuizTotal"] as? Number ?: 0).toInt()
        return quizTotal + trueFalseTotal + puzzleTotal + imageQuizTotal
    }

    private fun calculateOverallCorrectAnswers(
        currentProgress: Map<String, Any>,
        updatedData: Map<String, Any>
    ): Int {
        ...
        val newCorrectAnswer = when (currentExerciseType) {
            ExerciseType.QUIZ -> (updatedData["quizCorrect"] as? Number
                ?: currentQuizCorrect).toInt()

            ExerciseType.TRUE_FALSE -> (updatedData["trueFalseCorrect"] as? Number
                ?: currentTrueFalseCorrect).toInt()

            ExerciseType.PUZZLES -> (updatedData["puzzleCorrect"] as? Number
                ?: currentPuzzleCorrect).toInt()
        ...
        val newImageQuizCorrect = when (currentExerciseType) {
            ExerciseType.IMAGE_QUIZZES -> (updatedData["imageQuizCorrect"] as? Number
                ?: currentImageQuizCorrect).toInt()

            else -> currentImageQuizCorrect
        }
        // Depending on the type, replace the corresponding old value with the new one
        val totalQuizCorrect =
            if (currentExerciseType == ExerciseType.QUIZ) newCorrectAnswer else currentQuizCorrect
        val totalTrueFalseCorrect =
            if (currentExerciseType == ExerciseType.TRUE_FALSE) newCorrectAnswer else currentTrueFalseCorrect
        val totalPuzzleCorrect =
            if (currentExerciseType == ExerciseType.PUZZLES) newCorrectAnswer else currentPuzzleCorrect
